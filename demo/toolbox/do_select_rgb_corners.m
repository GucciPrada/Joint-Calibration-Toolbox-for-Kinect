...
if(isempty(use_automatic))
  use_automatic = true;
  corner_count_x = input(['Inner corner count in X direction ([]=10): ']); % 10 with Wei's Checkerboard
  corner_count_y = input(['Inner corner count in Y direction ([]=7): ']); % 7 with Wei's Checkerboard
else
  use_automatic = false;
end
...
for k=1:ccount
  ...
  default = 6;
  % default to 6 when using 1280 * 1024 image, therefore, 
  % we choose 3 for 640*480 images
