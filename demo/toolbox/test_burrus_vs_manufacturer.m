...
for i=find(~cellfun(@isempty,depth_plane_mask))
  ...
  burrus_calib = burrus_reg_data_export(burrus_reg_data);
...
%%
  dataset_labels = {'Burrus''s method','Manufacturer calibration'};
  bins = 64;
...
if ispc
    export_fig(sprintf('C:\\Users\\xw_11_000\\Dropbox\\wei\\SIGCHI2015LaTex\\figures\\burrus_vs_manu_depth_uncertainty_%s', str_dataset), '-pdf', '-nocrop', h);
elseif isunix
    export_fig(sprintf('/home/wei/Dropbox/wei/SIGCHI2015LaTex/figures/burrus_vs_manu_depth_uncertainty_%s', str_dataset), '-pdf', '-nocrop', h);
end
  %%
