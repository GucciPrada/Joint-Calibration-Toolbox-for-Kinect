...
fprintf('                  %c[%3.2f   %3.2f]\n', ...
  pm, depth_calib_error.dK(1,1),depth_calib_error.dK(2,2));
end
...
if(~isempty(depth_calib_error))
fprintf('                  %c[%3.2f   %3.2f]\n', ...
  pm, depth_calib_error.dK(1,3),depth_calib_error.dK(2,3));
end

fprintf('  Distortion:      [%3.4f   %3.4f   %3.4f   %3.4f   %3.4f]\n', ...
  depth_calib.dkc);
if(~isempty(depth_calib_error))
fprintf('                  %c[%3.4f   %3.4f   %3.4f   %3.4f   %3.4f]\n', ...
  pm, depth_calib_error.dkc);
end
...
fprintf('  Depth params:    [%3.2f   %3.6f]\n', ...
  depth_calib.dc);
% if(~isempty(depth_calib_error))
% % fprintf('                    %c[%3.6f   %3.2f   %3.4f]\n', ...
% %   pm, depth_calib_error.dc);
% fprintf('                  %c[%3.6f   %3.2f]\n', ...
%   pm, depth_calib_error.dc);
% end

varlist = [];
varlist = getStructFields(depth_calib_error, varlist);
if(any(strcmp(varlist,'dc')))
% fprintf('                    %c[%3.6f   %3.2f   %3.4f]\n', ...
%   pm, depth_calib_error.dc);
fprintf('                  %c[%3.6f   %3.2f]\n', ...
  pm, depth_calib_error.dc);
...
% fprintf('               %c[%3.5f   %3.5f   %3.5f]\n', ...
%   pm, depth_calib_error.Rrel);
%end
