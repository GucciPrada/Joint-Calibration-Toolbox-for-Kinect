...
function [K,R,t]=calib_from_homographies(H)
  ...
  for i=1:hcount
    Hi = vH{i}/norm(vH{i}(:,1)); % normalization confirmation--we have normalized before
    V(2*i-1,:) = vij(Hi,1,2)';
  ...
  if b(1) < 0           % to be consistant, following rule to flip the sign
      b(1) = -b(1);     % modified by WeiXiang,
  end                   % needs to be verified!
  %v0 = (B12B13 ? B11B23)/(B11B22 ? B212)
  v0 = (b(2)*b(4)-b(1)*b(5)) / (b(1)*b(3)-b(2)^2);
  %? = B33 ? [B213 + v0(B12B13 ? B11B23)]/B11
  lambda = b(6) - (b(4)^2+v0*(b(2)*b(4)-b(1)*b(5)))/b(1);
  lambda = abs(lambda); % modified
  %? =sqrt ?/B11
  alpha = (lambda/b(1))^0.5;
  %? =sqrt ?B11/(B11B22 ? B212)
  
  beta = (lambda*b(1)/(b(1)*b(3)-b(2)^2))^0.5; % original 
  beta = (lambda*b(1)/abs((b(1)*b(3)-b(2)^2)))^0.5; % modified
  %c = ?B12?2?/?
