...
burrus_reg_data = read_yaml([ret_path 'burrus_freenect_1.17433.yaml']);
frustum = read_frustum([ret_path 'teichman_reg.dat']);
...
for i=find(~cellfun(@isempty,depth_plane_mask))
  ...
  %% Export Teichman's reg data
  teichman_calib = teichman_reg_data_export();
  xw = disparity2world(u,v,disparity,teichman_calib);
  xw(3,:) = techiman_undistort(xw(3,:), u, v, frustum);
...
%%
  dataset_labels = {'Teichman''s method','Manufacturer calibration'};
  bins = 64;
...
if ispc
    export_fig(sprintf('C:\\Users\\xw_11_000\\Dropbox\\wei\\SIGCHI2015LaTex\\figures\\teichman_vs_manu_depth_uncertainty_%s', str_dataset), '-pdf', '-nocrop', h);
elseif isunix
    export_fig(sprintf('/home/wei/Dropbox/wei/SIGCHI2015LaTex/figures/teichman_vs_manu_depth_uncertainty_%s', str_dataset), '-pdf', '-nocrop', h);
end
