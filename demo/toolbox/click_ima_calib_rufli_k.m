...
%   USA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [corner_p]=click_ima_calib_rufli_k(kk,I,use_corner_find,wintx,winty,n_sq_x,n_sq_y)

global draw_rgb_corners


auto_path = [fileparts(mfilename('fullpath')) '/autoCornerFinder'];
map = gray(256);
...
fclose('all');
if exist([auto_path '/cToMatlab/cornerInfo.txt'],'file')
    delete([auto_path '/cToMatlab/cornerInfo.txt']);
end
if exist([auto_path '/cToMatlab/cornersX.txt'],'file')
    delete([auto_path '/cToMatlab/cornersX.txt']);
end
if exist([auto_path '/cToMatlab/cornersY.txt'],'file')
    delete([auto_path '/cToMatlab/cornersY.txt']);
end
if exist([auto_path '/cToMatlab/error.txt'],'file')
    delete([auto_path '/cToMatlab/error.txt']);
end
...
% fid = fopen([auto_path '/pictures.txt'],'w');
% fprintf(fid,'../%s',l(kk).name);
% fclose(fid);
% iter_r = iter_r + 1;    %!!!iter_r is defined in "click_calib.m"!!!

%Save image for external program.
%'temp.jpg' should always be in pictures.txt

% Notably, must write as BMP if using system("FindCorners -vars")
imwrite(I,[auto_path '/temp.bmp']);
...
if ~ispc %if not Windows
    callString = './';
    callString = ([callString 'FindCorners -w ' num2str(n_sq_x+1) ' -h ' num2str(n_sq_y+1) ' -m ' num2str((n_sq_x+1) * (n_sq_y+1)) ' pictures.txt']);
else
    callString = ([callString 'FindCorners.exe -w ' num2str(n_sq_x+1) ' -h ' num2str(n_sq_y+1) ' -m ' num2str((n_sq_x+1) * (n_sq_y+1)) ' pictures.txt']);
end

% %
% % Visualization turned ON
% callString = (['cd autoCornerFinder & FindCornersVisual -w ' num2str(n_sq_x+1) ' -h ' num2str(n_sq_y+1) ' -m ' num2str((n_sq_x+1) * (n_sq_y+1)) ' pictures.txt']);
...
%****************************************************************************
% In Xcode, cvLoadImage can handle different types of images, however,
% when using system("FindCorners -vars") in Matlab, it can only work on BMP
% images, otherwise cvLoadImage will return NULL and show "segmentation fault".
%*************************************************************************/
if ~ispc % unix
    callBack   = system(callString);
else
    callBack   = dos(callString);
end
...
filename = [auto_path '/cToMatlab/cornerInfo.txt'];
fid = fopen(filename, 'r');
cornerInfo = fscanf(fid, '%g %g', [1 2]);
fclose(fid);

%Open the files with the found corners
filename = [auto_path '/cToMatlab/cornersX.txt'];
fid = fopen(filename, 'r');
...
filename = [auto_path '/cToMatlab/cornersY.txt'];
fid = fopen(filename, 'r');
...
for i = 1:1:rows
    for j = 1:1:(cols-deltaCols)
        if cornersX(i,j) ~= -1
            ...
            PlotCornerNumber = [PlotCornerNumber, cornerNumber+1];
            if use_corner_find
...
if draw_rgb_corners
    ...
    colormap(map);
    set(1,'color',[1 1 1]);
    % title({['Image ' num2str(kk)]; [num2str(numOfFoundCorners) ' / ' num2str(numCorners) ' corner have been found.']; ['Red: initial detection. Blue: enhanced']; ['Press ENTER to continue.']});
    title({['Image ' num2str(kk) '-' num2str(numOfFoundCorners) ' / ' num2str(numCorners) ' corner have been found. Red: initial detection. Blue: enhanced']});
    % h = get(gca, 'title');
    % set(h, 'FontWeight', 'bold')
    % axis([min_x max_x min_y max_y])
...
if numCorners ~= numOfFoundCorners
    fprintf('Couldn''t locate all corners automatically for image %d. Use manual selection instead.\n',kk);
    %fprintf(1,'Press ENTER to continue.\n');
    %pause;
    corner_p = [];
    corner_x = [];
...
% [dummy,sizex] = size(x);
% iNumber = 1:1:sizex;
% reposition_corner = input('Would you like to reposition any of the assigned corners (other = yes, [] = no)?','s');
...
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %Reorder so that corners are sorted by pixel coordinate
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% do_reorder = false;
% if(x(n_cor_y*(n_cor_x-1)+1) < x(1))
%   do_reorder = true;
%   ii = n_cor_x:-1:1;
% else
%   ii = 1:n_cor_x;
...
%   for j=1:n_cor_y
%     for i=1:n_cor_x
%       x((i-1)*n_cor_y + j) = xTemp((ii(i)-1)*n_cor_y + jj(j));
%       y((i-1)*n_cor_y + j) = yTemp((ii(i)-1)*n_cor_y + jj(j));
%     end
%   end
% end
...
% Xi = ones(1,n_sq_y+1)'*(0:n_sq_x);
% Yi = (0:n_sq_y)'*ones(n_sq_x+1,1)';
...
if 0
    ...
    imshow(I);
    colormap(map);
    title({['Image ' num2str(kk) ' The corners have been renumbered in the right order. Press ENTER to continue or other to cancel.']});
    h = get(gca, 'title');
    set(h, 'FontWeight', 'bold')
    hold on;
    %Plot the corners
    ...
    draw_axes(x', y',n_sq_y);
    res=input('Use these corners ([]=yes, other=discard):','s');
    if(~isempty(res))
        corner_x = [];
        corner_p = [];
    end
