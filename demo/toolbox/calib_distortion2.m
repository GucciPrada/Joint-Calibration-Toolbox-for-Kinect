function [alpha,im_beta]=calib_distortion2(calib, Rext, text, imd_dataset, depth_plane_mask)
  width = 640;
  height = 480;
...
%   cnt = 0;
  ...
  [points,disparity]=get_depth_samples2(imd_dataset, depth_plane_mask);
  for i = find(~cellfun(@isempty, imd_dataset))
      if isempty(Rext{i}) % depth images that were taken parallel with the wall
          continue; % however, thoese depth images are not used so far...
      end
      points_i = points{i};
      disparity_i = disparity{i};
      %     [points_i, disparity_i] = get_depth_samples2(imd_dataset, depth_plane_mask);
...
%     fprintf('cnt = %d\n', cnt);
  ...
  %Sort for beta optimization: why?
