...
int ind = 0; // image start index

#define OUTPUT_DIR "C:\\Users\\xw_11_000\\Dropbox\\research\\Kinect\\code\\demo\\A3\\"


template<int D> int getDigit(int val)       { return getDigit<D - 1>(val / 10); }
template<>      int getDigit<1>(int val)    { return val % 10; }

string gen_frame_number(const int ind)
{
	char buff[100];

	if (ind >= 0 && ind < 10)
	{
		sprintf_s(buff, sizeof(buff), "000%d", ind);
	}
	else if (ind >= 10 && ind < 100)
	{
		sprintf_s(buff, sizeof(buff), "00%d%d", getDigit<2>(ind), getDigit<1>(ind));
	}
	else if (ind >= 100 && ind < 1000)
	{
		sprintf_s(buff, sizeof(buff), "0%d%d%d", getDigit<3>(ind), getDigit<2>(ind), getDigit<1>(ind));
	}
	else if (ind >= 1000 && ind < 10000)
	{
		sprintf_s(buff, sizeof(buff), "%d%d%d%d", getDigit<4>(ind), getDigit<3>(ind), getDigit<2>(ind), getDigit<1>(ind));
	}
	else
	{
		std::cerr << "Format index number failed!" << std::endl;
	}

	string format_ind(buff);
                       ...
                       string& filename, bool& isFileReading )
{
    ...
    imageMode = 1; // default to output high resolution BGR image

    retrievedImageFlags[0] = false; // depth map
    retrievedImageFlags[1] = true;  // disparity map
    retrievedImageFlags[2] = false;	// valid pixels mask
    retrievedImageFlags[3] = true;	// rgb image
    retrievedImageFlags[4] = false;	// gray image
...
int main( int argc, char* argv[] )
{
    ...
    for(;;)
    {
        ...
        else
        {
			...
			if (retrievedImageFlags[1] && capture.retrieve(disparityMap, CV_CAP_OPENNI_DISPARITY_MAP_32F))
            {
                if( isColorizeDisp )
                {
		...
		else if (c == 32) // Space bar
		{
			...
			// add frame index
			output_path = output_path + gen_frame_number(ind);

			if (!bgrImage.empty())
			{
				string temp_str = output_path + "-c1.jpg";
				imwrite(temp_str, bgrImage);
			}
			if (!disparityMap.empty())
			{
				string temp_str = output_path + "-d.pgm";
				...
				imwrite(temp_str, disparityMap, compression_params);//save disparity map
